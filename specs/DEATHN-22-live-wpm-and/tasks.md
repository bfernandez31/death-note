...
- [ ] T001 Add metric constants (WPM_BUFFER_SIZE, WPM_WINDOW_SECONDS, SMOOTHING_FACTOR, WPM_HUD_X, WPM_HUD_Y, ACC_HUD_X, ACC_HUD_Y, METRICS_HUD_SIZE) after the existing score/combo constants block (~line 33) in src/main.zig
- [ ] T002 Add metric state variables (wpm_buffer, wpm_buffer_head, wpm_buffer_count, correct_chars, wrong_chars, elapsed_time, displayed_wpm, displayed_accuracy) as a new block after the score/combo state variables (~line 80) in src/main.zig
...
- [ ] T003 Implement recordCorrectTimestamp() function that pushes elapsed_time into the circular wpm_buffer, advancing wpm_buffer_head with modular wrap and capping wpm_buffer_count at WPM_BUFFER_SIZE, in src/main.zig
- [ ] T004 Implement countCharsInWindow() function that scans all valid wpm_buffer entries and returns the count of timestamps within the last WPM_WINDOW_SECONDS of current elapsed_time, in src/main.zig
- [ ] T005 Implement resetMetricsState() function that zeros wpm_buffer, wpm_buffer_head, wpm_buffer_count, correct_chars, wrong_chars, elapsed_time, sets displayed_wpm to 0.0 and displayed_accuracy to 100.0, in src/main.zig
- [ ] T006 Modify the input loop (~lines 152–176) in frame() to classify each keypress inside the while-loop: call typedMatchesAnyEnemy() per key — if true, call recordCorrectTimestamp(elapsed_time) and increment correct_chars; if false, increment wrong_chars and set combo_count to 0. Remove the post-loop mismatch check at lines 174–176 in src/main.zig
- [ ] T007 Add resetMetricsState() call in the game-over restart block (~line 300, alongside resetScoreState()) in src/main.zig
- [ ] T008 Add elapsed_time accumulation (elapsed_time += raylib.GetFrameTime()) per frame, gated by !is_game_over, placed after the is_transitioning countdown block (~line 229) in src/main.zig
- [ ] T009 Add unit tests for circular buffer wrap (fill > WPM_BUFFER_SIZE entries, verify head wraps and count caps at WPM_BUFFER_SIZE) and resetMetricsState (verify all fields return to initial values) at the end of the test block in src/main.zig
...
- [ ] T010 [US1] Add WPM sliding window unit test: set up 60 entries in wpm_buffer within a 10-second window, call calculateTargetWpm(), assert result equals 72.0 (60 × 1.2) in src/main.zig
- [ ] T011 [US1] Add WPM early-game unit test: set correct_chars to 12, elapsed_time to 5.0, call calculateTargetWpm(), assert result is approximately 28.8 (12 × 12 / 5) in src/main.zig
- [ ] T012 [US1] Add WPM zero-input unit test: with empty buffer and elapsed_time 0.0, call calculateTargetWpm(), assert result equals 0.0 in src/main.zig

### Implementation for User Story 1

- [ ] T013 [US1] Implement calculateTargetWpm() function: if elapsed_time < WPM_WINDOW_SECONDS use early-game formula (correct_chars × 12 / elapsed_time), else use sliding window formula (countCharsInWindow() × 1.2), returning 0 when elapsed_time is 0, in src/main.zig
- [ ] T014 [US1] Implement updateMetrics() function with WPM section: advance elapsed_time, compute target WPM via calculateTargetWpm(), apply smoothing (displayed_wpm += SMOOTHING_FACTOR × (target − displayed_wpm)), and call updateMetrics() per frame gated by !is_game_over (replacing the raw elapsed_time accumulation from T008) in src/main.zig
- [ ] T015 [US1] Add WPM HUD drawing inside the if (!is_game_over) block (~line 257, after combo HUD): format displayed_wpm as "WPM {d}" (rounded to integer) via bufPrintZ, draw with DrawText at (WPM_HUD_X, WPM_HUD_Y) with METRICS_HUD_SIZE and DARKGRAY color in src/main.zig
...
- [ ] T016 [US2] Add accuracy unit test: set correct_chars to 100, wrong_chars to 4, call calculateTargetAccuracy(), assert result is approximately 96.15 in src/main.zig
- [ ] T017 [US2] Add accuracy zero-input unit test: with correct_chars 0 and wrong_chars 0, call calculateTargetAccuracy(), assert result equals 100.0 in src/main.zig

### Implementation for User Story 2

- [ ] T018 [US2] Implement calculateTargetAccuracy() function: compute (correct_chars / (correct_chars + wrong_chars)) × 100.0 using float casts, returning 100.0 when both counters are zero, in src/main.zig
- [ ] T019 [US2] Extend updateMetrics() with accuracy section: compute target accuracy via calculateTargetAccuracy(), apply smoothing (displayed_accuracy += SMOOTHING_FACTOR × (target − displayed_accuracy)) in src/main.zig
- [ ] T020 [US2] Add accuracy HUD drawing inside the if (!is_game_over) block (below WPM HUD): format displayed_accuracy as "Acc {d}%" (rounded to integer) via bufPrintZ, draw with DrawText at (ACC_HUD_X, ACC_HUD_Y) with METRICS_HUD_SIZE and DARKGRAY color in src/main.zig
...
- [ ] T021 [US3] Add smoothing convergence unit test: set displayed_wpm to 0.0, simulate multiple updateMetrics() calls with a fixed target WPM of 72.0, verify displayed_wpm increases by 20% of the remaining gap each step (e.g., after 1 step: 14.4, after 2 steps: 25.92) in src/main.zig
...
- [ ] T022 [US3] Verify SMOOTHING_FACTOR constant is 0.2 and is used in both WPM and accuracy smoothing paths within updateMetrics() in src/main.zig — no code change expected if T014 and T019 are correct
...
- [ ] T023 Run zig build test and verify all existing + new unit tests pass
- [ ] T024 Run zig build and verify clean compilation with no warnings
